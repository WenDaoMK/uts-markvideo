...
test('cameraX uvue page owns UI and calls xyc-markvideo native camera methods', async () => {
  ...
  assert.doesNotMatch(page, /<view class="cameraDebugBorder" :style="cameraViewportStyleText"><\/view>/);
  ...
  assert.match(page, /checkRecordPermissions\(\)/);
  ...
  assert.match(page, /callNativeCameraMethod\(nativeCamera, 'preparePermissions'\)/);
  assert.match(page, /callNativeCameraMethod\(nativeCamera, 'prepareRecordPermissions'\)/);
  assert.match(page, /callNativeCameraMethod\(nativeCamera, 'checkRecordPermissions'\)/);
  assert.match(page, /this\.normalizeNativeCommandReturn\(callNativeCameraMethod\(nativeCamera, 'prepareRecordPermissions'\), '请先完成录像权限授权', '录像权限已准备', \['10'\]\)/);
  assert.match(page, /this\.normalizeNativeResult\(callNativeCameraMethod\(nativeCamera, 'checkRecordPermissions'\), '请先完成录像权限授权'\)/);
  assert.doesNotMatch(page, /prepareRecordPermissions\(true\)/);
  ...
  assert.match(page, /await callNativeCameraMethodWithArg\(nativeCamera, 'setFlashMode', mode\)/);
  assert.match(page, /callNativeCameraMethodWithArg\(nativeCamera, 'setZoomMode', mode\)/);
  ...
  assert.match(page, /async syncZoomMode\(mode(?:: string)?, silent(?:: boolean)?, requestId(?:: number)? = 0\)(?:: Promise<boolean>)? \{[\s\S]*this\.zoomLatestRequestMode = mode[\s\S]*this\.zoomRequestSilent = silent[\s\S]*const nativeResponse = await callNativeCameraMethodWithArg\(nativeCamera, 'setZoomMode', mode\)/);
  ...
  assert.match(page, /callNativeCameraMethod\(nativeCamera, 'switchCamera'\)/);
  ...
  assert.match(page, /callNativeCameraMethod\(nativeCamera, 'takePhoto'\)/);
  assert.match(page, /handlePhotoDone\(event: UTSJSONObject \| null\)/);
  assert.match(page, /拍照请求已受理|拍照中/);
  assert.match(page, /const optionsText = JSON\.stringify\(\{ fps: this\.targetFps \}\)[\s\S]*callNativeCameraMethodWithArg\(nativeCamera, 'startRecord', optionsText != null \? optionsText : '\{\}'\)/);
  assert.doesNotMatch(page, /nativeCamera\.startRecord\(\{ fps: this\.targetFps \}\)/);
  assert.match(page, /callNativeCameraMethod\(nativeCamera, 'stopRecord'\)/);
  assert.match(page, /WATERMARK_STORAGE_KEY/);
  assert.match(page, /watermarkTemplates/);
  ...
  assert.match(stopBranch, /const stopResponse = await callNativeCameraMethod\(nativeCamera, 'stopRecord'\)/);
  ...
  assert.match(page, /triggerHaptic\(type(?:: any \| null)?\)(?:: void)? \{[\s\S]*normalizeHapticType\(safeString\(type, 'light'\)\)[\s\S]*const nativeCamera = this\.resolveNativeCamera\(\)[\s\S]*callNativeCameraMethodWithArg\(nativeCamera, 'performHapticFeedback', hapticType\)/);
  ...
  assert.match(page, /if \(this\.mode === 'photo'\) \{[\s\S]*this\.triggerHaptic\('light'\)[\s\S]*callNativeCameraMethod\(nativeCamera, 'takePhoto'\)/);
  ...
  assert.match(page, /this\.triggerHaptic\('medium'\)[\s\S]*this\.isRecording = false[\s\S]*callNativeCameraMethod\(nativeCamera, 'stopRecord'\)/);
  ...
  assert.match(page, /normalizeNativeCommandReturn\(callNativeCameraMethod\(nativeCamera, 'prepareRecordPermissions'\), '请先完成录像权限授权', '录像权限已准备', \['10'\]\)/);
  assert.match(pressShutterBody, /if \(!await this\.prepareRecordPermissions\(\)\) \{[\s\S]*this\.scheduleRecordPermissionRetry\(\)/);
  assert.match(pressShutterBody, /this\.clearRecordPermissionRetry\(\)[\s\S]*await this\.startRecordWithReadyPermissions\(nativeCamera\)/);
  ...
  assert.match(startRecordReadyBody, /this\.recordStartPending = true[\s\S]*this\.startRecordPendingTimeout\(\)[\s\S]*const optionsText = JSON\.stringify\(\{ fps: this\.targetFps \}\)[\s\S]*callNativeCameraMethodWithArg\(nativeCamera, 'startRecord', optionsText != null \? optionsText : '\{\}'\)/);
  ...
  assert.match(page, /const payloadText = JSON\.stringify\(payload\)[\s\S]*normalizeNativeCommandReturn\(await callNativeCameraMethodWithArg\(nativeCamera, 'setWatermark', payloadText != null \? payloadText : '\{\}'\), '水印设置失败', '水印已更新', \['12', '14'\]\)/);
  assert.match(page, /normalizeNativeCommandReturn\(await callNativeCameraMethod\(nativeCamera, 'takePhoto'\), '拍照失败', '拍照中', \['10', '11', '13', '14'\]\)/);
  ...
  assert.match(page, /if \(this\.activeWatermark != null && !await this\.flushWatermarkSync\(true\)\) \{[\s\S]*return[\s\S]*callNativeCameraMethod\(nativeCamera, 'takePhoto'\)/);
  assert.match(page, /if \(this\.activeWatermark != null && !await this\.flushWatermarkSync\(true\)\) \{[\s\S]*return[\s\S]*const optionsText = JSON\.stringify\(\{ fps: this\.targetFps \}\)[\s\S]*callNativeCameraMethodWithArg\(nativeCamera, 'startRecord', optionsText != null \? optionsText : '\{\}'\)/);
  assert.doesNotMatch(page, /nativeCamera\.startRecord\(\{ fps: this\.targetFps \}\)/);
...
test('xyc-markvideo Android component bridges to native camera view', async () => {
  ...
  assert.match(android, /checkRecordPermissions\(\) : string/);
  assert.match(android, /return view\.checkRecordPermissions\(\)/);
...
test('xyc-markvideo Android native view uses camera preview, photo, and 30fps recording', async () => {
  ...
  assert.match(nativeView, /private fun albumOpenIntents\(mimeType: String\): ArrayList<Intent>/);
  assert.match(nativeView, /Intent\(Intent\.ACTION_MAIN\)/);
  assert.match(nativeView, /Intent\.CATEGORY_APP_GALLERY/);
  assert.match(nativeView, /val collectionUri = mediaCollectionUri\(mimeType\)/);
  assert.match(nativeView, /Intent\(Intent\.ACTION_PICK\)[\s\S]*\.setDataAndType\(collectionUri, mimeType\)/);
  assert.match(nativeView, /Intent\(Intent\.ACTION_VIEW\)[\s\S]*\.setDataAndType\(collectionUri, mimeType\)/);
  ...
  assert.doesNotMatch(nativeView, /lastPublishedAlbumUri/);
  assert.match(nativeView, /private fun rememberPublishedMedia\(albumResult: AlbumSaveResult, kind: String\)/);
  assert.doesNotMatch(nativeView, /private fun albumOpenUri\(mediaUri: String\): Uri/);
  assert.match(nativeView, /private fun mediaCollectionUri\(mimeType: String\): Uri/);
  assert.match(nativeView, /private fun albumOpenMimeType\(mediaUri: String\): String/);
  assert.doesNotMatch(nativeView, /return Uri\.parse\(preferredUri\)/);
  ...
  assert.match(onWindowFocusChangedBody, /if \(hasWindowFocus && recordPermissionRequested\) \{/);
  assert.match(onWindowFocusChangedBody, /val missingPermissions = recordMissingPermissions\(\)/);
  assert.match(onWindowFocusChangedBody, /clearRecordPermissionRequestState\(\)/);
  ...
  assert.match(nativeView, /fun checkRecordPermissions\(\): String/);
  assert.notEqual(checkRecordPermissionsBody, '', 'checkRecordPermissions body should be inspectable');
  assert.match(checkRecordPermissionsBody, /val missingPermissions = recordMissingPermissions\(\)/);
  assert.match(checkRecordPermissionsBody, /missingPermissions\.isEmpty\(\)[\s\S]*ok\(payload\(\)\.put\("message", "录像权限已准备"\)\)/);
  assert.match(checkRecordPermissionsBody, /\.put\("errorCode", "1003"\)/);
  assert.match(checkRecordPermissionsBody, /recordPermissionMessage\(missingPermissions\)/);
  assert.doesNotMatch(checkRecordPermissionsBody, /requestPermissions|requestRecordPermissionsIfNeeded|failAndEmit|emitError|setStatus/);
  assert.notEqual(scheduleRecordPermissionRetryBody, '', 'scheduleRecordPermissionRetry body should be inspectable');
  assert.match(scheduleRecordPermissionRetryBody, /if \(recordMissingPermissions\(\)\.isEmpty\(\)\) \{[\s\S]*recordPermissionRequested = false[\s\S]*recordPermissionRetryCount = 0/);
  assert.match(scheduleRecordPermissionRetryBody, /if \(recordPermissionRetryCount < CAMERA_PERMISSION_RETRY_LIMIT\) \{[\s\S]*scheduleRecordPermissionRetry\(\)/);
  assert.match(scheduleRecordPermissionRetryBody, /clearRecordPermissionRequestState\(\)[\s\S]*setStatus\("录像权限已准备"\)/);
  assert.match(scheduleRecordPermissionRetryBody, /recordPermissionRetryCount < CAMERA_PERMISSION_RETRY_LIMIT[\s\S]*clearRecordPermissionRequestState\(\)/);
  assert.notEqual(onWindowFocusChangedBody, '', 'onWindowFocusChanged body should be inspectable');
