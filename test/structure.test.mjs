...
test('xyc-markvideo Android native view uses camera preview, photo, and 30fps recording', async () => {
  ...
  assert.match(nativeView, /val invalidVideoReason = when \{[\s\S]*recorder\.videoSampleCount <= 0 -> "录像没有写入有效视频帧"[\s\S]*else -> null/);
  assert.doesNotMatch(nativeView, /hadFrameError -> "录像帧编码失败"/);
  assert.match(nativeView, /if \(invalidVideoReason != null\) \{[\s\S]*outputTarget\.discard\(context\)[\s\S]*failAndEmit\("1402", invalidVideoReason/);
  assert.match(nativeView, /Log\.e\([\s\S]*"record stop failed during finish: error=\$\{stopError\}; frames=\$\{recorder\.frameCount\}; videoSamples=\$\{recorder\.videoSampleCount\}; hadFrameError=\$\{hadFrameError\}; finishMs=\$\{finishMs\}"/);
  assert.match(nativeView, /Log\.e\([\s\S]*"record stop failed validation: reason=\$\{invalidVideoReason\}; frames=\$\{recorder\.frameCount\}; videoSamples=\$\{recorder\.videoSampleCount\}; hadFrameError=\$\{hadFrameError\}; finishMs=\$\{finishMs\}"/);
  assert.match(nativeView, /failAndEmit\("1402", invalidVideoReason, "frames=\$\{recorder\.frameCount\}; videoSamples=\$\{recorder\.videoSampleCount\}; hadFrameError=\$\{hadFrameError\}; finishMs=\$\{finishMs\}"/);
  ...
  assert.match(nativeView, /RECORD_AUDIO_ENABLED = false/);
  ...
  assert.match(nativeView, /if \(writeMuxerSample\(activeMuxer, videoTrackIndex, encodedData, bufferInfo\)\) \{[\s\S]*videoSampleCount \+= 1/);
  assert.match(nativeView, /private fun writeMuxerSample\([\s\S]*\): Boolean \{[\s\S]*if \(!muxerStarted \|\| trackIndex < 0\) return false[\s\S]*activeMuxer\.writeSampleData\(trackIndex, encodedData, bufferInfo\)[\s\S]*return true/);
