...
## [LRN-20260625-C11] best_practice

**Logged**: 2026-06-25T12:16:00+08:00
**Priority**: high
**Status**: resolved
**Area**: android-permissions

### Summary
CameraX 挂载阶段不能静默预请求麦克风权限，录像权限请求标记只能在系统请求真正发出后置位。

### Details
用户反馈新安装首次使用时一直提示麦克风权限，但没有弹出系统授权。项目 PRD 已约定“挂载只请求相机；开始录像时请求麦克风”，但 `handleCameraReady()` 里会在相机 ready 后调用 `prepareRecordPermissions(true)`，导致默认照片模式也提前碰麦克风权限。Android 原生层还在 `requestPermissions()` 真正拿到 Activity 并发起系统请求前就设置 `recordPermissionRequested = true`，如果当时没有成功 dispatch，后续点击录像会被该标记挡住，表现为反复提示业务权限文案但系统弹窗不再出现。

### Suggested Action
后续处理 CameraX 权限时，保持麦克风只由录像入口触发；不要在 `cameraready`、`onShow` 或 silent preflight 中调用 `prepareRecordPermissions(true)`。原生 `requestPermissions()` 应返回是否成功 dispatch，`cameraPermissionRequested` / `recordPermissionRequested` 只在返回 true 后置位并启动 retry。

### Metadata
- Source: user_feedback, debugging
- Related Files: pages/cameraX/index.uvue, uni_modules/xyc-markvideo/utssdk/app-android/XycNativeCameraView.kt, test/structure.test.mjs, docs/embedded-camera-component-prd.md
- Tags: cameraX, android-permissions, microphone, first-run, runtime-permission
- Pattern-Key: uts_markvideo.microphone_permission_user_action_only
- Recurrence-Count: 1
- First-Seen: 2026-06-25
- Last-Seen: 2026-06-25

### Resolution
- **Resolved**: 2026-06-25T12:16:00+08:00
- **Commit/PR**: pending
